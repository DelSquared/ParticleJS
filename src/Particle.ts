function Particle(x:number,y:number,vx:number,vy:number,ax:number,ay:number,life:number){
  this.x=x;
  this.y=y;
  this.vx=vx;
  this.vy=vy;
...
function ParticleFountain(x:number,y:number,rate:number,life:number,gravity:number,max:number){
  this.x=x;
  this.y=y;
  this.g=gravity;
  this.particles=[];
...
function RadialParticleVortex(x:number,y:number,max:number,radius:number){
  this.x=x;
  this.y=y;
  this.particles=[];
  this.max=max;
  ...
  this.create = function(){
    var x:number = this.x + this.r*Math.cos(2*Math.PI*Math.random());
    var y:number = this.y + this.r*Math.sin(2*Math.PI*Math.random());
  ...
  this.update=function(){
    ...
    for (var i:number=0;i<this.particles.length;i++){
      var a:number = 1/((this.x - this.particles[i].x)*(this.x - this.particles[i].x) + (this.y - this.particles[i].y)*(this.y - this.particles[i].y)+1);
